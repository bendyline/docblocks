...
**Your north star:** This codebase is primarily maintained by AI agents and a small team. Your job is to make it as legible, unambiguous, and high-quality as possible so those agents produce correct code on the first try. Duplicate code confuses agents. Inconsistent patterns cause agents to guess wrong. Missing types lead to runtime bugs across the IPC / postMessage / FileSystemProvider boundaries that no compiler catches. Stale or missing CLAUDE.md sends agents down dead-end paths. Every issue you find and fix is a future bug that never gets written.
...
These are the load-bearing patterns. There is **no CLAUDE.md** in the repo today — write one if it doesn't exist, and these should anchor it.
...
| "Claude skills"        | This skill set in `.claude/skills/`, plus whether a CLAUDE.md exists and is current                                                                                         |
...
CLAUDE.md                                # if it exists; otherwise note its absence
...
**CLAUDE.md exists at the repo root and is load-bearing — agents read it first.** Audit it on every review:
...
- **Cross-reference with this review:** Any "Critical Issue" in your scorecard that requires future-agent awareness needs a CLAUDE.md update. Don't fix the code and forget the doc.

If CLAUDE.md is missing (rare — should only happen on a freshly forked repo), create one covering: project pitch (markdown editor, four surfaces, FileSystemProvider abstraction, built on Squisq); build & test commands; the conventions (FileSystemProvider / DocBlocksHostAPI seams, no `any`, no `console.log`); the Squisq link/unlink workflow; the `app://` protocol invariant; known gotchas.
...
- [ ] CLAUDE.md reflects current architecture (or doesn't exist yet — propose creating it)
