...
import { NETWORK_TIMEOUT_MS, runGit, withRepoLock, type RunGitResult } from './exec.js';
...
export async function detectRepo(
  ...
  const commonDir = path.resolve(realRoot, (commonDirRaw ?? gitDirRaw).trim());
  ...
  return {
    detection: {
      isRepo: true,
      toplevel,
      rootIsToplevel: context.workspaceRoot === toplevel,
    },
    context,
  };
}
...
export async function status(ctx: RepoContext): Promise<GitResult<GitStatus>> {
  const scoped = ctx.workspaceRoot !== ctx.toplevel;
  const args = ['status', '--porcelain=v2', '--branch', '--untracked-files=all', '-z'];
  if (scoped) args.push('--', '.');
  const res = await runGit({ bin: ctx.gitBin, cwd: ctx.workspaceRoot, args });
...
export async function commit(
  ...
  return withRepoLock(ctx.toplevel, async () => {
    ...
    const committed = await runGit({ bin: ctx.gitBin, cwd: ctx.workspaceRoot, args: commitArgs });
    if (committed.code !== 0) {
...
export async function listBranches(ctx: RepoContext): Promise<GitResult<GitBranchInfo[]>> {
  ...
  return ok(parseForEachRef(res.stdout.toString('utf8')));
}
...
export async function checkoutBranch(ctx: RepoContext, name: string): Promise<GitResult<void>> {
  ...
  return withRepoLock(ctx.toplevel, async () => {
    const res = await runGit({ bin: ctx.gitBin, cwd: ctx.workspaceRoot, args: ['switch', name] });
...
export async function listRemotes(ctx: RepoContext): Promise<GitResult<GitRemoteInfo[]>> {
  const res = await runGit({ bin: ctx.gitBin, cwd: ctx.workspaceRoot, args: ['remote', '-v'] });
  ...
  for (const [name, urls] of byName) {
    ...
    const web = parseRemoteUrl(fetchUrl);
    remotes.push({ name, fetchUrl, pushUrl: urls.pushUrl, web });
  }
