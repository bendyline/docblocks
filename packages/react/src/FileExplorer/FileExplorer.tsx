...
export function FileExplorer({ provider, onSelect, onTreeChange, className }: FileExplorerProps) {
  ...
  const renderEntries = useCallback(
    (entries: FileSystemEntry[], depth: number): React.ReactNode => {
      return entries.map((entry) => (
        <FileTreeNode
          ...
          onDelete={tree.deleteEntry}
          onRename={tree.renameEntry}
    ...
    [tree, handleSelect, childEntries],
  );

  return (
    <div className={`db-file-explorer ${className ?? ''}`}>
      {/* Toolbar */}
